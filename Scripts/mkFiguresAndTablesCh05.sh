...
bname=""
# bname=multiplierArray # no default
...
USAGE="`basename $0` -[ypRrcChSBv] [-b WIDTH] [-P PIPE] [-T TECH] -N BASENAME \n\
     \t -h help  \n\
     ...
     \t -N BASENAME: The filename of the Verilog source without the .v extension, e.g. MultiplierArray\n\
...
while getopts cChpRrvySBb:P:T:F:N: c
 do
    case $c in
        y)  doYosys=Yes;;
        p)  doPandR=Yes;;
...
if [ _$bname = "" ]
then
    echo "Argument -N BASENAME is reuired".
    exit 2
fi

if [ \! -r $bname-Pipelined-0.v ]
then
    echo "Source file $bname.v is not found or not readable."
    exit 2
fi
...
if [ $doClean = "Yes" ]
then
    echo -ne "${hlc} $(tput bold)*** Cleaning up$(tput sgr0):${noc}"
    for d in $designDirs
    do
...
then
    ...
    echo " "
    echo -e "${hlc} $(tput bold)*** Synthesis$(tput sgr0):${noc}"
...
then
    echo " "; echo " "
    echo -e "${hlc} $(tput bold)*** Placement & Routing$(tput sgr0):${noc}"
...
then
    echo " "
    echo -ne "${hlc} $(tput bold)*** Making Summary Table$(tput sgr0)${noc}"
	...
	do
		...
		# Columns 4, 5 (Resources):
		case $tech in
		    osu018|osu035|osu035_redm4|osu050|gscl45nm)
			resFile=$bname-$dir-${w}b-$tech-Resources.txt
			if [ -r $resFile ]
			then nrGates=`awk '/GATE/ { print $3;} ' $resFile`
			else nrGates="0"
			fi ;;
		    lp1k|lp8k|hx1k|hx8k)
			resFile=$bname-$dir-${w}b-ice40-Resources.txt
			if [ -r $resFile ]
			then nrGates=`awk '/LUT4/ { print $3;} ' $resFile`
			else nrGate=0
			fi ;;
		esac

		if [ -r $resFile ]
		then nrDFFs=`awk '/DFF/ { print $3;} ' $resFile`
		else nrDFFs=0
		fi
		...
		# For some reason the criticl path model is different for the carry multiplier
		# than for the other two versions:
		if [ $bname = "multiplierCarry" ]
		then
		    case $dir in
			Pipelined-0) critPathModel=`echo "$dANDFF+(2*$w)*$dFA" | bc` ;;
			Pipelined-h) critPathModel=`echo "$dANDFF+($w)*$dFA" | bc` ;;
			Pipelined-2) critPathModel=`echo "$dANDFF+2*$dFA" | bc` ;;
			Pipelined-n) critPathModel=`echo "$dANDFF + $dFA" | bc` ;;
		    esac
		else
		    case $dir in
		...
		# Column 9: (frequency, synth):
		if (( $(echo "$critPath == 0" |bc -l) ))
		then freq=0
		else freq=`echo "scale=1; 1000/$critPath" | bc` # in MHz
		fi
		...
		case $dir in
		    Pipelined-0) e2eDelayModel=$critPathModel;;
