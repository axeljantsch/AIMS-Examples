...
then
	...
	fsmtexf=$bsf-fsm.tex
	echo " " >$fsmtexf
	echo "% Generated by $0" >>$fsmtexf
	echo "% FSM description for $bsf from yosys log file." >>$fsmtexf
	echo " " >>$fsmtexf

	echo "\\\\begin{lstlisting}[language=shellcode,basicstyle=\\\\tiny]" >>$fsmtexf
	
	extractInfoPart.sh $bsf.yosyslog fsm | egrep -v '^$' >>$fsmtexf
	echo "\\\\end{lstlisting}" >>$fsmtexf
	resultfiles=$resultfiles" "$fsmtexf
	
	#######################################################
	# Storing synthesis info:
	synttexf=$bsf-synth.tex
	echo " " >$synttexf
	echo "% Generated by $0" >>$synttexf
	echo "% Resource usage after synthesis for $bsf from yosys log file." >>$synttexf
	echo " " >>$synttexf

	echo "\\\\begin{lstlisting}[language=shellcode]" >>$synttexf
	
	extractInfoPart.sh $bsf.yosyslog stat >>$synttexf
	echo "\\\\end{lstlisting}" >>$synttexf
	resultfiles=$resultfiles" "$synttexf
